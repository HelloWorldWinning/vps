...
struct AppState {
    ...
    host_mappings: Vec<(PathBuf, PathBuf)>,
}
...
fn json_ok(path: String) -> Response {
    (
        StatusCode::OK,
        Json(SaveResp {
            saved_success: true,
...
impl IntoResponse for BadRequest {
    fn into_response(self) -> Response {
        json_err(StatusCode::BAD_REQUEST, self.0)
    }
}
...
impl IntoResponse for Unauthorized {
    fn into_response(self) -> Response {
        json_err(StatusCode::UNAUTHORIZED, "unauthorized".to_string())
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    if host_mappings.is_empty() {
        info!("HOST_MAPPING_PATHS: none");
    } else {
        // Log as "container -> host" for clarity
        let mappings_str = host_mappings
            .iter()
            .map(|(host, cont)| format!("{} -> {}", cont.display(), host.display()))
            .collect::<Vec<_>>()
            .join(", ");
        info!("HOST_MAPPING_PATHS: {}", mappings_str);
    }
...
async fn upload_multipart(
    ...
    if let Some(ct) = headers.get("content-type").and_then(|v| v.to_str().ok()) {
        ...
        if ct.starts_with("multipart/form-data") && !ct.contains("boundary=") {
            return Err(BadRequest(
                "multipart/form-data missing boundary; use curl -F 'file=@path'".to_string(),
            )
            .into_response());
        }
    }
    ...
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|_| BadRequest("invalid multipart form".to_string()).into_response())?
    {
    ...
    Err(BadRequest("missing multipart field 'file'".to_string()).into_response())
}
...
// ---------- Host path mapping helpers ----------

fn parse_host_mappings(list: &str) -> Vec<(PathBuf, PathBuf)> {
    // Accept ';' or ',' as separators between rules.
    list.split(|c| c == ';' || c == ',')
        .filter_map(|pair| {
            let pair = pair.trim();
...
fn map_to_host_path(path: &Path, mappings: &[(PathBuf, PathBuf)]) -> PathBuf {
    for (host_prefix, container_prefix) in mappings {
        if path.starts_with(container_prefix) {
            if let Ok(rest) = path.strip_prefix(container_prefix) {
