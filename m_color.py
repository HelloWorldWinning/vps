#!/usr/bin/env python3
"""
i / ii  --  ls -laF style listing with a tiktoken (gpt-4o) token count column,
              themed to look like `eza --icons -laF --sort=modified` (the `l` alias).

  i    sort by mtime, OLDEST first  -> newest modified file at the BOTTOM
  ii   sort by name, A-Z ascending
...
Install:
  sudo install -m 0755 i.py /usr/local/bin/i
  sudo ln -sf /usr/local/bin/i /usr/local/bin/ii   # same script, behaviour from argv[0]
...
CACHE_PATH = "~/.cache/i/tokens.json"
CACHE_MAX_ENTRIES = 20000
SHOW_TOTAL = True  # print a total-tokens footer
BATCH = 16  # files encoded per tiktoken batch call


TOKEN_HUMAN = True  # False -> back to 5,240
TOKEN_DECIMAL = False  # True -> 5.2k / 28k (eza's rule); False -> 5k / 28k
TOKEN_TOTAL_HUMAN = False  # footer total: keep it exact
...
class Theme:
    ...
    def tokens(self, s: str) -> str:
        ...
        return self.paint(num, C_TOKENS) + self.paint(unit, C_TOKENS_UNIT)
...
def get_encoder():
    ...
    try:
        ...
        print(f"i: tiktoken unavailable ({e}); token column disabled", file=sys.stderr)
        _ENC = None
    return _ENC
...
def collect(paths):
    ...
    for p in paths:
        try:
            ...
            print(f"i: {p}: {e.strerror}", file=sys.stderr)
            continue
...
def render(entries, theme: Theme):
        ...
        rows.append(
            {
                ...
                "tok": human_count(e.tok) if e.tok is not None else e.note,
                "uid": st.st_uid,
                "e": e,
            }
        )
    ...
    if SHOW_TOTAL:
        ...
        sys.stdout.write(
            ...
            + theme.paint(" file(s)  tokens ", C_FOOTER)
