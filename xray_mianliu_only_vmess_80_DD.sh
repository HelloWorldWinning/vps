...
curl -4  -sL https://get.acme.sh | sh -s email=xray_mianliu@wardao.eu.org
source ~/.bashrc
...
#    if [[ $result != "uid=0(root)" ]]; then
    ...
    if [[ "$?" != "0" ]]; then
        ...
        CMD_UPGRADE="apt update; apt upgrade -y; apt autoremove -y; apt install jq -y;"
    else
        PMT="yum"
...
echo -e " ${RED}伪装域名没有解析到当前服务器 $IPV4 $IPV6 "
exit 1
fi
...
#            fi
    ...
    if [[ "$(needNginx)" = "no" ]]; then
        if [[ "$TLS" = "true" ]]; then
            read -p " 请输入xray监听端口[强烈建议4443，默认4443]：" PORT
            [[ -z "${PORT}" ]] && PORT=4443
        else
        ...
        read -p " 请输入Nginx监听端口[100-65535的一个数字，默认80]：" PORT
        [[ -z "${PORT}" ]] && PORT=80
        if [ "${PORT:0:1}" = "0" ]; then
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        read -p "  请选择伪装网站类型[默认:https://www.google.com]" answer
        if [[ -z "$answer" ]]; then
...
menu() {
    ...
    echo -e "  ${RED}3.${PLAIN}   安装Xray-${BLUE}VMESS+WS${PLAIN}${RED}(免流)${PLAIN}"
    echo -e "  ${GREEN}33.${PLAIN}  安装Xray-${BLUE}trojan+WS${PLAIN}${RED}(免流)${PLAIN}"
