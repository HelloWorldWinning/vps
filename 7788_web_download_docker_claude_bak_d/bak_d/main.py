...
def get_file_info(path: str) -> dict:
    """Get file/directory information safely (handles symlinks and disappearing files)."""
    try:
        st = os.lstat(path)  # don't follow symlinks; safer for /dev, broken links, etc.
        ...
        # If an entry vanishes or can't be stat'ed, return placeholders
        return {
            "size": 0,
            "modified": "-",
        }
...
# List of text file extensions to render in the browser (as HTML pre-formatted)
...
# Mapping of text file extensions to MIME types (for proper charset handling)
TEXT_MIME_TYPES = {
    ".js": "text/javascript",
    ".mjs": "text/javascript",
...
@app.get("/{path:path}")
async def read_path(
    path: str, credentials: HTTPBasicCredentials = Depends(authenticate)
):
    ...
    # Map the URL path to the internal file system path under the mounted /Host directory
    internal_path = os.path.abspath(os.path.join(BASE_DIR, path))

    # Strong traversal protection: ensure resolved path stays under BASE_DIR
    ...
    if os.path.isdir(internal_path):
        try:
            ...
            # Directory vanished between checks
        ...
        if file_extension in TEXT_FILE_EXTENSIONS:
            ...
            # For text files, explicitly set charset=utf-8 to properly display Chinese and other Unicode characters
            if file_extension in TEXT_MIME_TYPES:
                mime_type = TEXT_MIME_TYPES[file_extension]
                return FileResponse(
                    internal_path, media_type=f"{mime_type}; charset=utf-8"
                )
            else:
