...
server {
    ...
    # Directly serve d.share at the root
    location / {
        root /usr/share/nginx/html;
        index nonexistent_index.html;  # Force directory listing by using non-existent index
        autoindex on;
...
create_docker_compose() {
    read -p "Enter docker-compose.yml full path (default: /root/nginx_docker_d): " DOCKER_COMPOSE_PATH
    if [[ -z "$DOCKER_COMPOSE_PATH" ]]; then
        DOCKER_COMPOSE_PATH="/root/nginx_docker_d"
    fi
    ...
    # Create a sample file in d.share
