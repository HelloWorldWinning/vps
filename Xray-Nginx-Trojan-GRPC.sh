...
temp_d=$(mktemp -d)
    ...
    if [ $get_arch = "x86_64" ];then
        ...
        mv -fv $temp_d/xray /usr/bin/xrayT
	...
	if [ -f "/usr/bin/xrayT" ]; then
		chmod 755 /usr/bin/xrayT
		echoColor purple "\nDownload completed."
	else
	...
	cat <<EOF > /etc/systemd/system/xrayT.service
[Unit]
...
ExecStart=/usr/bin/xrayT -c /etc/xrayT/config.json
Restart=on-failure
...
netstat  -lptnu |grep  $Port
           ...
           cat "/etc/xrayT/config.json" 
    systemctl daemon-reload
    systemctl enable xrayT
    systemctl start  xrayT
            systemctl restart xrayT
            systemctl status xrayT


}

echo -e "  ${GREEN}1.${PLAIN} 安装 ${BLUE}xrayT${PLAIN}"
