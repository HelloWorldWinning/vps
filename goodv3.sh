...
${Red_font_prefix}27${Font_color_suffix} dd后 新建 ~/.ssh,覆盖安装 ~/.ssh/authorized_keys sed -i "s/PasswordAuthentication yes/PasswordAuthentication no/g"  /etc/ssh/sshd_config ; systemctl restart sshd
