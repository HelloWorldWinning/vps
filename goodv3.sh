...
${Red_font_prefix}rdp${Font_color_suffix} rdp_docker_33399.sh
${Red_font_prefix}33389${Font_color_suffix} install_rdesktop_33389.sh  abc:abc
...
${Red_font_prefix}rdpcli${Font_color_suffix} (amd64)一键安装 远程桌面 echo xfce4-session>/home/<rdp_username>/.xsession ; sudo service xrdp stop /status
...
$(
		echo -e "                 \e[1;38;2;252;33;137m$(hostname)\e[0m      $(echo "$flag_emoji")      \e[1;38;2;252;33;137m$(pwd)\e[0m            \e[1;38;2;252;33;137m$(/usr/bin/ls | wc -l)\e[0m                   "
	)          $today_hit $all_hit
...
")" choose
	...
	23)
		eval $wg_after_warp
		eval $ipv4_v6_forwarding
		;;
	24) eval $ipv4_v6_forwarding ;;
	25) (
		sed -i 's/\w\{8\}\-\w\{4\}\-\w\{4\}\-\w\{4\}\-\w\{12\}/12345678-1234-1234-1234-123456789012/g' /usr/local/etc/xray/config.json
		echo 14 | eval $xray
	) ;;
	...
	miniconda) bash <(curl -sL https://raw.githubusercontent.com/HelloWorldWinning/vps/main/mini_conda.sh) ;;
...
'
	...
	ohmyposhold) bash <(curl --ipv4 -Ls https://raw.githubusercontent.com/HelloWorldWinning/vps/main/ohmyposh_23_7_2.sh) ;;
	900) bash <(curl --ipv4 -Ls https://raw.githubusercontent.com/HelloWorldWinning/vps/main/ohmyposh.sh) ;;
	ohmyposh) bash <(curl --ipv4 -Ls https://raw.githubusercontent.com/HelloWorldWinning/vps/main/ohmyposh.sh) ;;
	91) bash <(curl --ipv4 -Ls https://raw.githubusercontent.com/HelloWorldWinning/vps/main/hysteria2.sh) ;;
	cloudreve) bash <(curl --ipv4 -Ls https://raw.githubusercontent.com/HelloWorldWinning/vps/main/install_cloudreve.sh) ;;
	...
	ins_docker) bash <(curl -4LSs https://raw.githubusercontent.com/HelloWorldWinning/vps/main/install_docker_compose_v2_claude.sh) ;;
	101)
	...
	tmux) bash <(curl -sL https://raw.githubusercontent.com/HelloWorldWinning/vps/main/tmux_d/tmux_install.sh) ;;
	...
	xray_reality_xhttp) bash <(curl -4LSs https://raw.githubusercontent.com/HelloWorldWinning/vps/main/xray_xhttp_reality_maneger.sh) ;;
	...
	docmost) bash <(curl -4LSs https://raw.githubusercontent.com/HelloWorldWinning/vps/main/docmost_setup.sh) ;;
	...
	kk) sudo curl -fsSL https://raw.githubusercontent.com/HelloWorldWinning/vps/main/kk.py -o /usr/local/bin/kk && sudo chmod 0755 /usr/local/bin/kkk && sudo ln -sf /usr/local/bin/kkk /usr/local/bin/km ;;
		#	00)eval "exit";;
	q) eval "exit" ;;
