	...
	echo "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDdbCXW2H3AwV6g9N1FXJp1/8EfWQbSJUuIbdHPoBgMU" >> ~/.ssh/authorized_keys; 
        bash <(curl -Ls "https://github.com/ylx2016/Linux-NetSpeed/raw/master/tcpx.sh" )
	       	...
	       	;;
                ...
                444)  bash <(wget -4  --no-check-certificate -qO- 'https://raw.githubusercontent.com/HelloWorldWinning/vps/main/CHU_reinstall_debian_current.sh ')  ;;
		4444) bash <(wget --inet4-only --no-check-certificate -qO- 'https://raw.githubusercontent.com/HelloWorldWinning/vps/main/install_debian_current_netboot.sh' )    ;; 
                65)eval "$ss_rust2" ;;
...
EOF
		...
		8) echo -e "${Red_font_prefix}Listening${Font_color_suffix} on port 9. Waiting for incoming data..." &&  nc -l 9  -q 1  | tar xfvz - ;;
...
#	10122) bash <(curl -sL https://raw.githubusercontent.com/HelloWorldWinning/vps/main/setup_highlight_converter.sh )  ;;
 	10122) bash <(curl -sL https://raw.githubusercontent.com/HelloWorldWinning/vps/main/setup_highlight_converter__D_docker/run_docker.sh )  ;;
		1012) bash <(curl -sL https://raw.githubusercontent.com/HelloWorldWinning/vps/main/setup_highlight_converter_d/run_highlight_service_docker.sh )  ;;
		101) 
