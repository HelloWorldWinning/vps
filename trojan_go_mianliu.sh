...
getData() {
    ...
    if [[ "$can_change" != "yes" ]]; then
        ...
        echo -e "  ${RED}2. 伪装域名DNS解析指向当前服务器ip$IPV4$IPV6 ${PLAIN}"
...
read -p "需要自签域名，免流吗？默认no：" mianliu_zhengshu
...
read -p "输入nginx 监听port 默认8080:" PORT 
if   [[ -z "$PORT" ]]; then
        PORT=8080
fi
...
#read -p "输入websocket path 默认/ws:"  WSPATH
#if   [[ -z "$WSPATH" ]]; then
#        WSPATH="/ws"
#fi

read -p "输入nginx 监听port 默认8080:" Nginx_PORT 
...
menu() {
    ...
    echo -e "  ${GREEN}2.${PLAIN}  安装trojan-go+WS ${RED}免流${PLAIN}"
