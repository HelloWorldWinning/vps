...
def _strip_frontmatter(lines):
    """Return (title_or_None, remaining_lines) after consuming YAML ---."""
    ...
    return title, lines            # unclosed frontmatter — return as-is


def _first_node_label(lines):
    """Return the label of the first real node in a graph/flowchart."""
    for ln in lines:
        # quoted label: ID["label"]  ID("label")  ID{"label"}
        ...
        if m and m.group(1) not in _KEYWORDS:
            return m.group(2).strip()
        # bare node ID (first word on an indented line)
        m = re.match(r"\s+(\w+)", ln)
        if m and m.group(1) not in _KEYWORDS:
            return m.group(1)
    return None
