...
# Configuration
DOCKER_IMAGE="docmost/docmost:latest"
DOCMOST_DIR="/data/docmost"
...
restart_docmost() {
    log "Restarting Docmost containers..."

    if [[ ! -d "$DOCMOST_DIR" ]]; then
        log_error "Docmost directory not found: $DOCMOST_DIR"
        return 1
    fi
...
cleanup_old_images() {
    log "Cleaning up old Docmost images..."

    local old_images
    old_images=$(docker images --filter "dangling=true" --filter "reference=docmost/docmost" -q 2>/dev/null || true)
...
main() {
    log "Starting Docmost update check..."
    ...
    if [[ "$needs_restart" == true ]]; then
        ...
        if restart_docmost; then
            log_success "✓ Successfully updated Docmost to latest version"
            cleanup_old_images
        else
            log_error "✗ Failed to restart Docmost"
            exit 1
        fi
    else
