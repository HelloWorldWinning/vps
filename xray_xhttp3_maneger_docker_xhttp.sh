...
# XHTTP3 (xhttp) stream config for Xray
...
EOF
	...
	echoColor green "Xray configuration created (XHTTP3 mode)."
}

# XHTTP3 client config
function generateClientConfig() {
	...
	# VLESS-XHTTP3 link format with HTTP/3 support
	VLESS_LINK="vless://${uuid}@${domain}:${port}?encryption=none&security=tls&sni=${domain}&alpn=h3%2Ch2&fp=chrome&type=xhttp&path=${client_path}&mode=stream-one#XHTTP3-${domain}"
...
===== Xray QUIC/XHTTP3 (VLESS-XHTTP3-Nginx Docker) Connection Information =====
Domain: $domain
Server IP: $ip
Port: $port
UUID: $uuid
...
EOF
	...
	echo ""
	echoColor yellow "===== VLESS Connection Link (XHTTP3 over QUIC) ====="
	echo "$VLESS_LINK"
	echo ""
...
function installXrayXHTTP3() {
	...
	checkDocker || {
		echoColor red "Docker check failed."
		return 1
	}
	...
	installAcme || {
		echoColor red "acme.sh install failed."
		return 1
	}
	...
	if [[ "$NEED_ISSUE" == "true" ]]; then
		issueCertificate "$DOMAIN" || {
			echoColor red "Cert issuance failed."
			return 1
		}
	fi
	...
	echoColor green "Proto  : VLESS-XHTTP3 (HTTP/3 QUIC)"
	...
	if docker compose ps --status running | grep -q "$XRAY_CONTAINER"; then
		echoColor green "Xray XHTTP3/QUIC started."
...
function startService() {
	[[ -f "$COMPOSE_FILE" ]] || {
		echoColor red "Compose file not found."
		return 1
	}
	cd "$BASE_DIR"
	docker compose up -d
...
function regenerateClientConfig() {
	...
	[[ -f "$XRAY_CONF_DIR/config.json" ]] || {
		echoColor red "Xray config not found."
		return 1
	}
	...
	local path
	# Try to get path for XHTTP3 config
...
function reissueCertificate() {
	...
	[[ -z "$domain" ]] && {
		echoColor red "Domain required."
		return 1
	}

	issueCertificate "$domain" || {
		echoColor red "Cert issue failed."
		return 1
	}
...
function showCertificateInfo() {
	...
	local days_left=$(((expiry_epoch - now_epoch) / 86400))
	if [[ $days_left -lt 30 ]]; then
		echoColor red "⚠ $days_left remaining! Consider renewing."
	else
		echoColor green "✓ $days_left remaining"
	fi
...
function showMenu() {
	clear
	echoColor green "=============================================="
	echoColor green "   Xray XHTTP3 Manager v1.2 (Docker)"
	echoColor green "   (VLESS-XHTTP3-Nginx with HTTP/3 QUIC)"
	echoColor green "=============================================="
	echo ""
	if checkInstallation; then
	...
	echo ""
	echoColor blue "  1. Install Xray XHTTP3 (Docker)"
...
######  end of xray_xhttp3_maneger_docker_xhttp.sh
