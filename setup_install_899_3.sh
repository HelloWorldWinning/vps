...
backup() {
  ...
  while [[ -e "$target" ]]; do n=$((n + 1)); target="${source}.bak.${stamp}.${n}"; done
  cp -a -- "$source" "$target"
  log "Backup: $target"
}
...
[[ "$BASE_URL" == */ ]] || { BASE_URL="${BASE_URL}/"; warn "Added trailing slash to BASE_URL."; }
...
# Managed by $SCRIPT_NAME. Add persistent custom settings below as needed.
...
# Keep all upstream defaults and enable additional general-web providers.
# Google Web is added explicitly because current defaults include Google CSE,
# but not the direct Google Web engine. Provider anti-bot limits may still
# suspend an individual engine; SearXNG will continue with the others.
