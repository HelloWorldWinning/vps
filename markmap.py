...
//const FOCUS_OFFSET_X_EXPAND = -530;
const FOCUS_OFFSET_X_EXPAND = -490;
...
.mm-shift-nav-box {
  fill: rgba(153, 60, 29, 0);
  stroke: none;
  vector-effect: non-scaling-stroke;
  pointer-events: none;
  filter: drop-shadow(4px 0 10px rgba(153, 60, 29, 0.16));
}

.mm-shift-nav-left-bar {
  fill: #993c1d;
  stroke: none;
  pointer-events: none;
}
...
function showActiveSelectionHud(node, extraLevels = null, timeout = 900) {
  if (!node) return;

  const visibleExtraLevels =
    extraLevels === null || extraLevels === undefined
      ? getVisibleExtraLevels(node)
      : Math.max(0, Number(extraLevels) || 0);

  const label = getNodeText(node) || "active node";

  showHud(`Active +${visibleExtraLevels}: ${label}`, timeout);
}
...
function clearNavHighlight() {
  document
    .querySelectorAll(
      ".mm-shift-nav-box, .mm-shift-nav-left-bar, .mm-shift-nav-scope-box"
    )
    .forEach(el => el.remove());
...
function expandOneVisibleLevel(root) {
  ...
  // Collect first. Do NOT mutate while walking,
  // otherwise opening root can make traversal continue into children
  // in the same d press.
  forEachVisibleNode(root, node => {
    if (!hasChildren(node)) return;

    // A visible folded node is the next expandable frontier
    // for that visible branch.
...
showActiveSelectionHud(node, extraLevels, 900);
...
function renderActiveSelection(mm) {
  ...
  function afterRender() {
    setTimeout(() => {
      ...
      showActiveSelectionHud(activeSelection.node, visibleExtraLevels, 900);
...
function drawTargetBox(el) {
  ...
  el
    .querySelectorAll(".mm-shift-nav-box, .mm-shift-nav-left-bar")
  ...
  bar.setAttribute("class", "mm-shift-nav-left-bar");
  bar.setAttribute("x", box.x - padX - barGap - barWidth);


const barPadTop = 0.85;
const barPadBottom = 5.8;  

bar.setAttribute("y", box.y - padY + barPadTop);
bar.setAttribute("height", box.height + padY * 2 - barPadTop - barPadBottom);
  ...
  el.insertBefore(bar, bg.nextSibling);
}
