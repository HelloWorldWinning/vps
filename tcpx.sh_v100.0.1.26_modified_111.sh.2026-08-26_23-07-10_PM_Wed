...
cat >/root/ultimate_wrapper.sh <<'EOF'
#!/bin/bash
# Set all environment variables to avoid interaction
export DEBIAN_FRONTEND=noninteractive
...
# Create wrapper scripts that ALWAYS say yes and use non-interactive options
mkdir -p /tmp/wrappers

cat > /tmp/wrappers/apt << 'WRAPPEREOF'
#!/bin/bash
yes | /usr/bin/apt -y -qq -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" "$@"
WRAPPEREOF

cat > /tmp/wrappers/apt-get << 'WRAPPEREOF'
#!/bin/bash
yes | /usr/bin/apt-get -y -qq -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" "$@"
WRAPPEREOF

cat > /tmp/wrappers/dpkg << 'WRAPPEREOF'
#!/bin/bash
yes | /usr/bin/dpkg --force-confdef --force-confold "$@"
WRAPPEREOF

chmod +x /tmp/wrappers/apt
chmod +x /tmp/wrappers/apt-get
chmod +x /tmp/wrappers/dpkg

# Put our wrappers first in PATH
export PATH="/tmp/wrappers:$PATH"

# Create a wrapper function for bash to pipe 'yes' to any command with apt
function_to_add='
yes_to_apt() {
  if [[ "$1" == "apt" || "$1" == "apt-get" || "$1" == "dpkg" ]]; then
    yes | command "$@"
  else
    command "$@"
  fi
...
# Add this function to a temporary .bashrc
echo "$function_to_add" > /tmp/temp_bashrc
echo 'alias apt="yes_to_apt apt"' >> /tmp/temp_bashrc
echo 'alias apt-get="yes_to_apt apt-get"' >> /tmp/temp_bashrc
echo 'alias dpkg="yes_to_apt dpkg"' >> /tmp/temp_bashrc

# Force bash to use our temporary .bashrc
export BASH_ENV=/tmp/temp_bashrc

# Run the script with all our safeguards in place
echo "Running pre_tcpx.sh with strongest non-interactive settings..."
bash --rcfile /tmp/temp_bashrc /root/pre_tcpx.sh < <(yes)

# Clean up
rm -rf /tmp/wrappers /tmp/temp_bashrc
EOF

chmod +x /root/ultimate_wrapper.sh
...
# Use a simple cron entry with maximum non-interactive settings for the initial sudo install
cron_job="@reboot sleep 20 ; yes | DEBIAN_FRONTEND=noninteractive apt-get -y -o Dpkg::Options::=\"--force-confdef\" -o Dpkg::Options::=\"--force-confold\" install sudo && sudo tmux new-session -d -s ins -c /root '/root/ultimate_wrapper.sh' #${unique_id}"
...
start_menu() {
 ...
 ${Green_font_prefix}111.${Font_color_suffix} 31 17 13 22 21
————————————————————————————————————————————————————————————————" &&
		check_status
	get_system_info
	...
	111)
		...
		apt_full_upgrade_no_stuck() {
			local timeout_limit="${1:-3600}" # default: 3600 seconds
			local kill_after="${2:-30}"      # SIGKILL after 30 seconds if SIGTERM fails
			...
			0)
				...
				;;
			124)
				echo "ERROR: apt full-upgrade timed out and was terminated."
				;;
			137)
				echo "ERROR: apt full-upgrade was force-killed."
				;;
			*)
				echo "ERROR: apt full-upgrade exited with code $rc."
				;;
			esac
		...
		apt_full_upgrade_no_stuck 1800 10
