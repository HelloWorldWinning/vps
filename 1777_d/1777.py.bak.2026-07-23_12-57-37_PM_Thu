...
def create_breadcrumb(path: str) -> str:
    ...
    for part in parts:
        if part:  # Skip empty parts
            # Skip the "Host" part in the breadcrumb
            if part == "Host":
                continue

            # For creating URLs, we need to track if we're in the root path
            if not current_path:
                current_path = part
            else:
                current_path = os.path.join(current_path, part)

            # Create URL path without 'Host'
...
async def navigate(
    ...
    if os.path.isdir(full_path):
        ...
        #   ?s     : sort by file size, biggest first
        #   ?t     : sort by modified time, latest first
        sort_mode = None
        ...
        is_raw = "raw" in request.query_params
        raw_suffix = "?raw" if is_raw else ""
        ...
        def item_sort_key(item: str):
            ...
            if sort_mode == "size":
                ...
                return (0, -stats.st_size if stats else 0, item.lower())

            if sort_mode == "time":
                return (0, -stats.st_mtime if stats else 0, item.lower())
                ...
                <tr>
                    <td>
                        <div class="item-container">
                            ...
                            <a href="/{parent_url}{raw_suffix}" class="folder">../</a>
                        </div>
                    </td>
        ...
        for item in items:
            ...
            if os.path.isdir(item_full_path):
                item_display = f"{item}/"
                item_class = "folder"
                item_suffix = raw_suffix  # propagate ?raw into subfolders too
            else:
                item_display = item
                item_class = "file"
                item_suffix = raw_suffix  # files open directly as plain text
                ...
                <tr>
                    <td>
                        <div class="item-container">
                            ...
                            <a href="/{item_url}{item_suffix}" class="{item_class}">{html.escape(item_display)}</a>
                        </div>
                    </td>
...
body * {{
    font-family: "SF Pro", -apple-system, BlinkMacSystemFont,
                 "WebSFPro", "PingFang SC", "FZFangJunHeiS",
                 sans-serif;
}}

/* only code-like / fixed-width metadata uses Source Code Pro */
pre,
pre *,
code,
code *,
.size-col,
.index-number {{
    font-family: "Source Code Pro", "FZFangJunHeiS",
...
table td:nth-child(3) {{ width: 15%; }}   
        ...
        if "raw" in request.query_params:
            return await serve_raw_text(full_path)

        ext = os.path.splitext(full_path)[1].lower()
        # Markdown: render with mistune + Pygments (env-configurable style)
        if ext == ".md":
            return await render_markdown_with_mistune(full_path, credentials)
...
async def serve_raw_text(full_path: str) -> PlainTextResponse:
    """Return the file as raw plain text with no rendering at all."""
...
code,
code * {
    font-family: "Source Code Pro", "SF Pro", -apple-system, BlinkMacSystemFont,
                 "PingFang SC", "FZFangJunHeiS", monospace !important;
}
...
async def render_markdown_with_mistune(
    full_path: str, credentials: HTTPBasicCredentials
):
    if not os.path.isfile(full_path):
        raise HTTPException(status_code=404, detail="File not found")
    ...
    # Pick the Pygments style for Markdown code blocks from env, default to current theme
    style_name = os.getenv("markdown_theme", "manni")
    try:
        style = get_style_by_name(style_name)
    except ClassNotFound:
        style_name = "manni"
    ...
    class HighlightRenderer(mistune.HTMLRenderer):
        def __init__(self, **kwargs):
            ...
            self.formatter = HtmlFormatter(linenos=True, style=style_name)
        ...
        # Compatible with Mistune v3 TOC heading ids.
        # Also harmless for normal Markdown headings.
        def heading(self, text, level, tid=None, **attrs):
            heading_id = attrs.get("id") or tid
            if heading_id:
                return (
                    f'<h{level} id="{html.escape(str(heading_id), quote=True)}">'
                    f"{text}"
                    f"</h{level}>\n"
                )
...
/* ============================================================
   Body headings ONLY
   Do not visually apply these heading styles to TOC items.
============================================================ */
...
.markdown-body > h1 {
    ...
    background-color: rgb(158, 34, 65);
    color: #FFFFFF !important;
...
/* ============================================================
   Mistune TOC: No Box / No Card
============================================================ */

.markdown-body .toc,
.markdown-body details.toc {
    display: block;
    margin: 1.2em 0 1.6em 0;

    /* remove the visible box */
    padding: 0 !important;
...
/* Remove Mistune's built-in TOC title */
.markdown-body .toc > h1,
.markdown-body .toc > h2,
.markdown-body .toc > h3,
.markdown-body .toc > h4,
.markdown-body .toc > summary,
.markdown-body details.toc > summary {
    display: none !important;
}

/* Reset TOC lists only */
.markdown-body .toc ul,
.markdown-body details.toc ul {
...
/* Top-level TOC item */
.markdown-body .toc > ul > li > a,
.markdown-body details.toc > ul > li > a {
    font-weight: 700 !important;
}

/* Nested TOC indentation */
.markdown-body .toc ul ul,
.markdown-body details.toc ul ul {
...
/* Third-level TOC */
.markdown-body .toc ul ul ul a,
.markdown-body details.toc ul ul ul a {
    font-size: 14px;
...
/* ============================================================
   Mistune TOC hover: text-only background, full-row hover kept
============================================================ */

.markdown-body {
    --toc-hover-pink: #ec4899;
}

/* TOC list rows become clipping containers */
.markdown-body .toc li,
.markdown-body details.toc li {
    position: relative;
    overflow: hidden;
...
/* TOC links: visual box only wraps the text */
.markdown-body .toc a,
.markdown-body details.toc a {
...
/* Invisible extension of the anchor's hover/click area */
.markdown-body .toc a::after,
.markdown-body details.toc a::after {
    content: "";
    position: absolute;
    top: 0;
    left: 0;

    width: 100vw;       /* extends hover area to the right */
    height: 100%;
...
/* Hover: background paints only the real text-sized anchor */
.markdown-body .toc a:hover,
.markdown-body details.toc a:hover,
.markdown-body .toc a:focus-visible,
.markdown-body details.toc a:focus-visible {
...
"""
    ...
    <head>
        ...
        <link rel="icon" type="image/x-icon" href="/static/favicon.ico">
        <link href="https://fonts.googleapis.com/css2?family=Source+Code+Pro:wght@400;700&display=swap" rel="stylesheet">
        <style>{HtmlFormatter(style=style_name, linenos=True).get_style_defs()}</style>
        <style>{custom_css}</style>
    </head>
    <body style="background-color: {style.background_color};">
