ifrom flask import Flask, render_template_string, request, session, redirect, url_for
import nbformat
import nbconvert
import os
...
#           return f'<html><head><title>{os.path.basename(file_path)}</title><style>{formatter.get_style_defs()}</style></head><body style="background-color: {style.background_color};">{highlighted_code}</body></html>'
          # Adding custom CSS for "Andale Mono" font
            custom_css = 'body, pre { font-family: "Andale Mono", monospace; }'
            custom_css += '.highlight .n { color: #57E857; }'  # Setting color for class=n
