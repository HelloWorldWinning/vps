...
EOF
	...
	echoColor green "Xray configuration created."
}
...
function generateClientConfig() {
	...
	VLESS_LINK="vless://${uuid}@${domain}:${port}?encryption=none&security=tls&sni=${domain}&alpn=h3&fp=chrome&type=xhttp&path=${client_path}&mode=stream-one#XHTTP3-${domain}"
...
===== Xray XHTTP3 (VLESS-XHTTP3-Nginx Docker) Connection Information =====
Domain: $domain
Server IP: $ip
Port: $port
UUID: $uuid
...
EOF
	...
	echo ""
	echoColor yellow "===== VLESS Connection Link (XHTTP3) ====="
	echo "$VLESS_LINK"
	echo ""
...
function installXrayXHTTP3() {
	echoColor yellow "===== Installing Xray XHTTP3 (Docker) ====="
	...
	if docker compose ps --status running | grep -q "$XRAY_CONTAINER"; then
		echoColor green "Xray XHTTP3 started."
...
function showStatus() {
	echoColor yellow "===== Xray XHTTP3 (Docker) Status ====="
...
function uninstallXray() {
	echoColor red "===== Uninstall Xray XHTTP3 (Docker) ====="
...
function showMenu() {
	clear
	echoColor green "=============================================="
	echoColor green "   Xray XHTTP3 Manager v1.1 (Docker)"
