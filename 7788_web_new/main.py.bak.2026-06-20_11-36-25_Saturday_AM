...
def get_sort_mtime(path: str) -> float:
    """Return modified timestamp for sorting. Missing/inaccessible files go last."""
    try:
...
def build_directory_rows(
    path: str, internal_path: str, items: Iterable[str]
) -> List[str]:
    table_rows: List[str] = []
    ...
    for idx, item in enumerate(items, 1):
        ...
        if os.path.isdir(item_internal_path):
            ...
            item_size = (
                format_size(item_info["size"]) if item_info["modified"] != "-" else "-"
            )
            item_class = "file"
...
def render_directory(
    path: str, internal_path: str, s: str | None = None
) -> HTMLResponse:
    try:
    ...
    if s is None:
        ...
        # ?s=1 means latest Modified at bottom
        items.sort(
            key=lambda item: (
                get_sort_mtime(os.path.join(internal_path, item)),
                item.casefold(),
            )
        )
    else:
        # ?s or ?s=0 means latest Modified at top
        items.sort(
            key=lambda item: (
                -get_sort_mtime(os.path.join(internal_path, item)),
                item.casefold(),
...
async def read_path(
    ...
    if os.path.isdir(internal_path):
        return render_directory(normalized_path, internal_path, s)
