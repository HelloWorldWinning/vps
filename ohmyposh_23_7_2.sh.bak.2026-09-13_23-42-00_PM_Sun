...
# The crontab entry we want to add (with flock)
CRON_ENTRY="*/1 * * * * /usr/bin/flock -n $LOCK_FILE $UPDATE_SCRIPT"

# Check if the crontab entry already exists (check for the script name)
if crontab -l | grep -Fq "$UPDATE_SCRIPT"; then
	echo "Crontab entry already exists. Updating with flock..."
	# Remove old entry and add new one with flock
	crontab -l | grep -v "$UPDATE_SCRIPT" | {
		cat
		echo "$CRON_ENTRY"
	} | crontab -
	echo "Crontab entry updated with flock."
else
