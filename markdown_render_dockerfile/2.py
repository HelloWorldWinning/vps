...
# Change this to point to the /Host directory
MARKDOWN_DIR = "/Host"
...
class StrikethroughExtension(Extension):
    def extendMarkdown(self, md):
        md.inlinePatterns.register(
            StrikethroughInlineProcessor(r"~~(.+?)~~"), "strikethrough", 175
        )
...
def list_files(subpath=""):
    ...
    # Build path display without showing /Host prefix
    if subpath:
...
if __name__ == "__main__":
    app.run(debug=False, host="0.0.0.0", port=178)
