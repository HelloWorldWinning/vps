...
# --- Normalize one user-entered path line ---
# Fixes the usual typing/paste artifacts:
#   "  file1  "     -> file1         (leading/trailing spaces and tabs)
#   "file1<CR>"     -> file1         (CRLF paste)
#   '"my file.txt"' -> my file.txt   (surrounding double quotes)
#   "'my file.txt'" -> my file.txt   (surrounding single quotes)
#   "my\ file.txt"  -> my file.txt   (shell drag-and-drop / tab-completion escaping)
#
# A rewritten form is only preferred when it actually exists on disk, so
# real filenames containing quotes, backslashes or trailing spaces still work.
# Prints the empty string for a blank or whitespace-only line.
normalize_input_path() {
	local raw="$1"
	local trimmed unquoted unescaped c
	...
	case "$unescaped" in
	*\\*)
		# Drop a backslash only when it escapes a non-alphanumeric char.
		unescaped=$(printf '%s' "$unquoted" | sed 's/\\\([^A-Za-z0-9]\)/\1/g')
		;;
	esac

	for c in "$unescaped" "$unquoted" "$trimmed" "$raw"; do
		[ -n "$c" ] || continue
		if [ -e "$(expand_path "$c")" ]; then
			printf '%s\n' "$c"
			return 0
		fi
	done

	# Nothing matched — return the most likely intent so the
	# "not a valid file or directory" warning shows something sane.
	printf '%s\n' "$unescaped"
}
...
while IFS= read -r raw_item; do
	item="$(normalize_input_path "$raw_item")"

	# A blank OR whitespace-only line ends the input.
	[ -z "$item" ] && break

	# Skip duplicates silently (same path entered twice).
	dup=0
	...
	if [ "$dup" -eq 1 ]; then
		echo -e "${YELLOW}Note: duplicate ignored:${NC} ${item}"
		continue
	fi

	# Only announce rewrites that were more than whitespace trimming
	# (i.e. quote stripping or backslash unescaping).
	raw_trimmed="${raw_item//$'\r'/}"
	raw_trimmed="${raw_trimmed#"${raw_trimmed%%[![:space:]]*}"}"
	raw_trimmed="${raw_trimmed%"${raw_trimmed##*[![:space:]]}"}"
	if [ "$item" != "$raw_trimmed" ]; then
		echo -e "${CYAN}Note: read as:${NC} ${item}"
	fi
