...
load_netbird_status_once() {
	...
	if printf '%s' "$NB_JSON" | jq -e '.peers.details | length > 0' >/dev/null 2>&1; then
		NB_HAS_PEERS=1
	fi
...
get_current_peer() {
	...
	if [ -z "$peer" ]; then
		peer="$(hostname -s 2>/dev/null || hostname 2>/dev/null || printf 'unknown')"
	fi
...
# --- Function to resolve a peer No. from the cached NetBird list to its NetBird IP ---
