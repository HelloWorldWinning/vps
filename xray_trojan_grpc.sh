...
mkdir -p /etc/xrayTrojanXtlsGrpc/
...
function DownloadxrayTrojanXtlsGrpcCore(){
	...
	echo -e "The Latest xrayTrojanXtlsGrpc version:"`echoColor red "${version}"`"\nDownload..."
    get_arch=`arch`
...
temp_d=$(mktemp -d)
    ...
    if [ $get_arch = "x86_64" ];then
        ...
        mv -fv $temp_d/xray /usr/bin/xrayTrojanXtlsGrpc
	...
	if [ -f "/usr/bin/xrayTrojanXtlsGrpc" ]; then
		chmod 755 /usr/bin/xrayTrojanXtlsGrpc
		echoColor purple "\nDownload completed."
	else
	...
	cat <<EOF > /etc/systemd/system/xrayTrojanXtlsGrpc.service
[Unit]
Description=xrayTrojanXtlsGrpc Service
Documentation=https://github.com/XTLS/xrayTrojanXtlsGrpc-core/
...
ExecStart=/usr/bin/xrayTrojanXtlsGrpc -c /etc/xrayTrojanXtlsGrpc/config.json
Restart=on-failure
...
netstat  -lptnu |grep $Port
           ...
           echo "/etc/xrayTrojanXtlsGrpc/config.json" 
           cat "/etc/xrayTrojanXtlsGrpc/config.json" 
    systemctl daemon-reload
    systemctl enable xrayTrojanXtlsGrpc
    systemctl start  xrayTrojanXtlsGrpc
            systemctl restart xrayTrojanXtlsGrpc
            systemctl status xrayTrojanXtlsGrpc


}

echo -e "  ${GREEN}1.${PLAIN} 安装 ${BLUE}xrayTrojanXtlsGrpc${PLAIN}"
...
read -p " 选择：" answer
    case $answer in
        1)
            Xray_Grpc
            DownloadxrayTrojanXtlsGrpcCore
            start
            ;;
        2)
           echo "/etc/xrayTrojanXtlsGrpc/config.json" 
