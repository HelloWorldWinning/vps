...
if [ $status -gt 128 ] || [ -z "$input" ]; then
    ...
    # Modified: Ask for filename with option to use original filename
    read -t 10 -p "Filename to save data (leave empty to use original filename): " filename
    # Default value is empty which will trigger using original filename
    ...
    temp_meta="$temp_dir/metadata"
    
    # First receive the metadata (the filename)
    nc -l -p 9 > "$temp_meta"
  ...
  # print_status "Receiving file data..."
    ...
    original_filename=$(cat "$temp_meta")
    
    # Determine final filename to use
    if [ -z "$filename" ]; then
        filename="$original_filename"
        print_status "Using original filename: $filename"
    fi
    ...
    if [ $is_ip_or_domain -eq 0 ]; then
      ...
      # print_status "Receiving file data..."
        ...
        # Extract the original filename from metadata (if specified, otherwise use provided)
        if [ -s "$temp_meta" ]; then
            original_filename=$(cat "$temp_meta")
            
            # If provided filename is actually "auto" or empty, use original
            if [ "$filename" = "auto" ] || [ -z "$filename" ]; then
                filename="$original_filename"
                print_status "Using original filename: $filename"
            fi
        fi
        ...
        print_status "Starting file transfer to $ip..."
        
        # Record start time for sending
        start_time=$(date +%s.%N)
        
        # First send the basename of the file (just the filename without path)
        basename "$filename" | nc -q 1 "$ip" 9
        print_status "Metadata sent. Sending file data..."
        
        # Then send the actual file data on port 10
        if command -v pv &> /dev/null; then
            pv "$filename" | nc -q 1 "$ip" 10
            nc_status=${PIPESTATUS[1]}
        else
            print_status "Sending file... Please wait..."
            nc -q 1 "$ip" 10 < "$filename"
            nc_status=$?
        fi
