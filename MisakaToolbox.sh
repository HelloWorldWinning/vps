...
version="v3.2"
version_log="闭关回归，脚本地址迁移，删除部分脚本"
...
warp_script(){
	...
	case $warpNumberInput in
        1) wget -N --no-check-certificate https://gitlab.com/misakablog/warp-script/-/raw/main/warp.sh && bash warp.sh ;;
...
xui() {
    ...
    echo "1. 使用X-ui官方原版"
    echo "2. 使用Misaka魔改版 (闭源)"
    echo "3. 使用FranzKafkaYu魔改版 (闭源)"
    echo "0. 返回主菜单"
    ...
    case "$xuiNumberInput" in
        ...
        2) wget -N --no-check-certificate https://gitlab.com/misakablog/x-ui/-/raw/main/install.sh && bash install.sh ;;
...
menu1(){
    ...
    echo -e " ${GREEN}10.${PLAIN} Tmux 后台任务管理"
    ...
    case $menuInput in
        1) open_ports ;;
        2) wget -N --no-check-certificate https://gitlab.com/misakablog/rootpwd/-/raw/main/root.sh && bash root.sh ;;
        3) wget -N --no-check-certificate https://gitlab.com/misakablog/screen-script/-/raw/main/screen.sh && bash screen.sh ;;
        4) bbr_script ;;
        5) v6_dns64 ;;
        6) warp_script ;;
        7) curl -fsSL https://get.docker.com | bash -s docker --mirror Aliyun ;;
        8) wget -N --no-check-certificate https://gitlab.com/misakablog/acme-script/-/raw/main/acme.sh && bash acme.sh ;;
        9) wget -N --no-check-certificate https://gitlab.com/misakablog/argo-script/-/raw/main/argo.sh && bash argo.sh ;;
        10) wget -N --no-check-certificate https://gitlab.com/misakablog/tmux-script/-/raw/main/tmux.sh && bash tmux.sh ;;
        11) bash <(curl -sSL https://cdn.jsdelivr.net/gh/SuperManito/LinuxMirrors@main/ChangeMirrors.sh) ;;
        12) setChinese ;;
        13) wget -N --no-check-certificate https://gitlab.com/misakablog/warp-script/-/raw/main/tun.sh && bash tun.sh ;;
        *) exit 1 ;;
    esac
}
...
menu3(){
    ...
    case $menuInput in
        ...
        5) wget -N --no-check-certificate https://gitlab.com/misakablog/xray-script/-/raw/main/xray.sh && bash xray.sh ;;
