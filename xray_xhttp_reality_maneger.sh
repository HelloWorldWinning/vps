...
BLUE="\033[36m"
PLAIN='\033[0m'
CONFIG_DIR="/etc/xray_reality_xhttp"
CONFIG_FILE="$CONFIG_DIR/config.json"
SERVICE_FILE="/etc/systemd/system/xray_reality_xhttp.service"
BINARY_PATH="/usr/bin/xray"
DATA_DIR="/data/xray_config_d_xhttp"
...
[Unit]
Description=Xray Reality XHTTP Service
...
function installXrayReality() {
	echoColor yellow "===== Installing Xray Reality XHTTP ====="
	...
	echoColor green "Network: xhttp"
	echoColor green "Path: /cloud/"
...
EOF
	...
	systemctl enable xray_reality_xhttp
	systemctl start xray_reality_xhttp

	sleep 2

	if systemctl is-active --quiet xray_reality_xhttp; then
		echoColor green "✓ Xray Reality XHTTP installed and started successfully!"
		generateClientConfig "$PUBLIC_IP" "$Port" "$UUID" "$PUBLIC_KEY"
	else
		echoColor red "✗ Failed to start Xray Reality XHTTP service"
		systemctl status xray_reality_xhttp
	fi
}
...
function generateClientConfig() {
	...
	# Generate VLESS link for XHTTP
	VLESS_LINK="vless://${uuid}@${ip}:${port}?encryption=none&security=reality&sni=www.oracle.com&fp=chrome&pbk=${pubkey}&type=xhttp&path=%2Fcloud%2F#XrayReality-XHTTP-${ip}"
...
EOF
	...
	cat <<EOF >$INFO_FILE
===== Xray Reality XHTTP Connection Information =====
Server IP: $ip
Port: $port
UUID: $uuid
Public Key: $pubkey
...
function showStatus() {
	echoColor yellow "===== Xray Reality XHTTP Status ====="
	...
	if checkInstallation; then
		...
		echo ""
		systemctl status xray_reality_xhttp --no-pager
		...
		echoColor red "✗ Xray Reality XHTTP is not installed"
	fi
}
...
function updateXray() {
	...
	systemctl stop xray_reality_xhttp
...
function stopService() {
	systemctl stop xray_reality_xhttp
	echoColor green "Xray Reality XHTTP service stopped"
}

function restartService() {
	systemctl restart xray_reality_xhttp
	sleep 1
	if systemctl is-active --quiet xray_reality_xhttp; then
		echoColor green "✓ Xray Reality XHTTP service restarted successfully!"
	else
...
function uninstallXray() {
	echoColor red "===== Uninstalling Xray Reality XHTTP ====="
	...
	systemctl stop xray_reality_xhttp
	systemctl disable xray_reality_xhttp
	rm -f $SERVICE_FILE
	rm -rf $CONFIG_DIR
	systemctl daemon-reload

	echoColor green "Xray Reality XHTTP uninstalled (config backup kept in $DATA_DIR)"
	echoColor yellow "Note: Xray binary at $BINARY_PATH was NOT removed (may be shared with other services)"
}

function showMenu() {
	clear
	echoColor green "======================================="
	echoColor green "   Xray Reality XHTTP Manager v1.2"
	echoColor green "======================================="
	echo ""
	...
	echo ""
	echoColor blue "  1. Install Xray Reality XHTTP"
...
while true; do
	...
	1)
		if checkInstallation; then
			echoColor yellow "Xray Reality XHTTP is already installed!"
